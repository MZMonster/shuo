...
!function ($) {
  ...
  function include(path, type){
    ...
    if (!BASE && path.indexOf('http') !== 0) {
      var src = "shuo.api.js"; // 当前js文件名
  ...
  function config(options) {
    ...
    // inject render.js
    include(OPTIONS.render || 'shuo.render.js');
  ...
  /**
   * @api {GET} shuo.api.groupOfCmts(sourceIDs,callback) D 批量获取文章的评论
   * @apiName groupOfCmts
   * @apiGroup shuo.api.js
   * @apiDescription 批量获取文章的评论，一般用于文章列表中，参考魅玩帮的讨论功能
   *
   * @apiParam {Array} sourceIDs thread的sourceID数组
   * @apiParam {Function} callback
   * @apiParamExample example:
   ...
   *   });
   */
  function groupOfCmts(sourceIDs, callback) {
    var data = {
      appid: OPTIONS.appid,
      sourceIDs: sourceIDs
    };
