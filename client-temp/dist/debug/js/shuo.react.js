...
!function ($) {
  ...
  function load(callback) {
    var ids = [];
    $('[data-shuo-star="true"]').attr('data-id', function (i, id) {
      ids.push(id);
    });
    if (!ids.length) {
      return;
    ...
    ajaxLoad(commentID, function(data){
      ids.forEach(function (id) {
        var $item = $('[data-id="' + id +'"][data-shuo-star="true"]');
        $item.removeClass('MW-star-init');
        if (data[id]) {
          $item.find(OPTIONS.numSelector).text(data[id].star);
          if (data[id].hasStarred) {
            //$item.addClass('MW-star-hasStarred');
            $item.attr('data-has-starred','true');
          }
        }
      });
  ...
  function ajaxLoad(commentID,callback) {
    if (!commentID) {
      return;
    }
